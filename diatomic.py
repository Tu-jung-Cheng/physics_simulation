...
class CO_molecule:
    ...
    def com(self): # return position of center of mass
        return (self.O.pos*self.O.m+self.C.pos*self.C.m)/(self.O.m+self.C.m) #(m1x1+m2x2)/(m1+m2)
    def com_v(self): # return velocity of center of mass
        return (self.O.v*self.O.m+self.C.v*self.C.m)/(self.O.m+self.C.m) #(m1v1+m2v2)/(m1+m2)
    def v_P(self): # return potential energy of the bond for the vibration motion
        return 0.5*self.bond.k*(mag(self.bond.axis)-d)**2 #1/2kx^2
    def v_K(self): # return kinetic energy of the vibration motion
        vc = (self.O.v*self.O.m+self.C.v*self.C.m)/(self.O.m+self.C.m)
        return 0.5*self.O.m*dot((self.O.v-vc),norm(self.bond.axis))*dot((self.O.v-vc),norm(self.bond.axis))+0.5*self.C.m*dot((self.C.v-vc),norm(self.bond.axis))*dot((self.C.v-vc),norm(self.bond.axis))
            #1/2m1(vc1)^2+1/2m2(vc2)^2
    def r_K(self): # return kinetic energy of the rotational motion
        vc = (self.O.v*self.O.m+self.C.v*self.C.m)/(self.O.m+self.C.m)
        normal_O = (self.O.v-vc)-dot((self.O.v-vc),norm(self.bond.axis))*norm(self.bond.axis)
        normal_C = (self.C.v-vc)-dot((self.C.v-vc),norm(self.bond.axis))*norm(self.bond.axis)
        return 0.5*self.O.m*dot(normal_O,normal_O)+0.5*self.C.m*dot(normal_C,normal_C)
    def com_K(self): #return kinetic energy of the translational motion of the center of mass
        return 0.5*(self.O.m+self.C.m)*mag2((self.O.v*self.O.m+self.C.v*self.C.m)/(self.O.m+self.C.m))
